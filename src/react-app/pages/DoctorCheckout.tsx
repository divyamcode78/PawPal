...
import { CheckCircle2, Dog, Stethoscope, Calendar, Clock, ArrowLeft, ArrowRight } from 'lucide-react';
...
export default function DoctorCheckoutPage() {
  ...
  const visit = params.get('visit') || 'checkup';
  const date = params.get('date') || '';
  const time = params.get('time') || '';
  const price = Number(params.get('price') || '0');

  const summary = useMemo(() => ({ petId, visit, date, time, price }), [petId, visit, date, time, price]);
  ...
  const placeOrder = async () => {
    ...
    if (!summary.petId || !summary.date || !summary.time || !summary.price) {
      setErrorMsg('Missing appointment details');
      return;
    }
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        let msg = 'Failed to place appointment';
        try {
      ...
      // Persist to localStorage so it shows in My Appointments
      try {
      ...
      navigate(`/doctor/confirm/${created.id}`, { state: { appointment: created } as any });
    } catch (e) {
      const optimistic = {
        id: 'temp',
        pet_id: summary.petId,
      ...
      // Persist optimistic appointment to localStorage
      try {
        const key = 'doctor_appointments';
        const existing = JSON.parse(localStorage.getItem(key) || '[]');
        const next = Array.isArray(existing) ? existing : [];
      ...
      navigate('/doctor/confirmed', { state: { appointment: optimistic } as any });
      return;
    } finally {
      setPlacing(false);
    }
        ...
        <div className="bg-white/70 backdrop-blur-lg rounded-2xl p-6 shadow-lg border border-white/20">
          <div className="flex items-center justify-between mb-6">
            ...
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <Stethoscope className="w-6 h-6 text-blue-500" /> Doctor Appointment Checkout
            </h1>
            <div className="w-8" />
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl border border-gray-200 bg-white">
              ...
              <div className="text-sm text-gray-700">Visit: <span className="font-semibold">{VISIT_LABELS[summary.visit] || summary.visit}</span></div>
            </div>
            <div className="flex items-center gap-3 p-4 rounded-xl border border-gray-200 bg-white">
          ...
          <div className="mt-6">
            ...
            <div className="flex gap-3">
              {([
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full border text-sm ${paymentMethod === m.id ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 bg-white text-gray-700 hover:bg-gray-50'}`}
                >
                  {m.label}
                </button>
              ))}
            ...
            {paymentMethod === 'cash' && (
              <div className="mt-4 text-sm text-gray-600">Pay with cash at the clinic on arrival.</div>
            )}
          </div>
